...
# Agricultural Mapping with Digital Earth Africa and the Satellite Image Time Series Package
...
```{r read geoarrow, echo=TRUE, message=FALSE, warning=FALSE}
...
```{r sits_mosaic, echo=TRUE, message=FALSE, warning=FALSE, fig.cap="True-colour image of Digital Earth Africa Sentinel-2 Geomedian for an area of Rwanda."}
mosaic <- sits_mosaic(
  cube = dea_s2_cube,
  crs = "EPSG:6933",
  output_dir = "data",
...
```{r sitstorast, echo=TRUE, message=FALSE, warning=FALSE}
...
```{r sits_train, echo=TRUE, message=FALSE, warning=FALSE}
...
```{r reduce imbalance, echo=TRUE, message=FALSE, warning=FALSE}
...
```{r classify, echo=TRUE, warning=FALSE, message=FALSE}
gc()
...
```{r classifymap, echo=TRUE, message=FALSE, progress=FALSE, warning = FALSE, fig.cap="Labelled crop groups from a classification over a region of Rwanda."}

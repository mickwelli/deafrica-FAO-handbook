...
download.file("https://ewocstorage.blob.core.windows.net/public/2020_rw_wapor_1_point_111/2020_RWA_FAO-WAPOR-1_POINT_111.geoparquet", method="curl", destfile='data/2020_rwa.geoparquet')
```
...
## Load Digital Earth Africa data
...
```{r load_gm, echo=TRUE, message=FALSE, warning=FALSE}
...
```{r falsecol, echo=TRUE, fig.cap="False colour image of Digital Earth Africa Sentinel-2 Geomedian over Rwanda."}
...
```{r sits_mosaic, echo=TRUE, message=FALSE, fig.cap="True-colour image of Digital Earth Africa Sentinel-2 Geomedian for an area of Rwanda."}
mosaic <- sits_mosaic(
  cube = dea_s2_cube,
  crs = "EPSG:6933",
  output_dir = "data",
...
```{r loadstac, echo=TRUE, message=FALSE, warning=FALSE, fig.cap="Cropland extent map from Digital Earth Africa for an area of Rwanda."}
crop_mask = list()
...
```{r create_crop_cube, echo=TRUE, message=FALSE, warning=FALSE}
crop_mask_local <- write_stars(crop_mask_mos, "data/crop_mask_20190101.tif", overwrite=TRUE)

cm_rast <- rast("data/crop_mask_20190101.tif")
...
```{r export_mask_rast, echo=TRUE, warning=FALSE, message=FALSE}
dir.create('data/tiles')
dir.create('data/masked')

makeTiles(mosaic_rast_masked, project(vect(sf), '+proj=cea +lat_ts=30 +lon_0=0 +x_0=0 +y_0=0 +datum=WGS84 +units=m +no_defs +type=crs'),filename="data/tiles/mosaic_rast_tile_.tif")

for (x in 1:length(list.files('data/tiles'))){
...
```{R sitscube_masked, echo=TRUE, message=FALSE, warning=FALSE}
...
```{R sitscube_masked_plot, echo=TRUE}
...
The overall accuracy statistic of 61.4% might be considered acceptable, especially given the large number (13) of classes and class imbalance. However, some classes, such as beans and sorghum, have quite low precision, recall and F1 values, which indicate how accurately individual classes are classified. If there was a particular interest in mapping these specific crop types, further class imbalance treatments or other approaches, such as introducing further predictor variable, might be considered. In this case, the next step will be combining classes to reduce issues associated with class imbalance.

```{r kfoldcrossval, echo=TRUE, message=FALSE, warning=FALSE}
...
```{r rfor_comb, echo=TRUE, warning=FALSE, message=FALSE}
